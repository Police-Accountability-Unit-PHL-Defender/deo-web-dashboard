...
- Refresh (note: the upstream README names a `generate_csvs.py` that does not exist):
  ...
  ```
- Read by: nothing at build time. It is the source of two published artifacts:
  - `public/cubes/districts.json` — PSA rows aggregated to district. Verified:
    PSAs 011 + 012 sum to district 01's total 38405 / white 20664 / 53.8%.
  - `POPULATION_BY_RACE` in `utils/snapshot.ts` — the five citywide totals.
  Neither is regenerated by any script, so changing this file requires
