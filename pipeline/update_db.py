...
from update_db_models import ProcessZip

HERE = Path(__file__).resolve().parent
DATA_DIR = HERE / "data"
ZIP_FILENAME = os.environ.get("ZIP_FILENAME", "").replace(":", "_")
...
@click.option("--zip", "zip_filename", default=None, help="Backup zip in pipeline/data/")
def cli(debug, remap_districts, most_recent_quarter_override, zip_filename):
    zip_filename = zip_filename or ZIP_FILENAME
    if not zip_filename:
        raise SystemExit("Pass --zip <file>")
    try:
        run = ProcessZip(
            ...
            zip_filename_override_dict={
                # Keyed "{table}/{year}" so it survives the backup job's CSV
                # naming changes. Needed due to the removal of most 2022
                # mvc_codes in Jan 2025.
                "car_ped_stops/2022": "car_ped_stops_2024-10-24T01_17_41.zip"
            },
